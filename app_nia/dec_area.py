...
class yoloDetect:
    ...
    def __init__(self, url, loc):
        ...
        self.model = attempt_load("models/pt/area/best.pt", map_location=self.device)  # 模型文件
        ...
        #     ('rtsp://', 'rtmp://', 'http://'))
        self.img_det = cv2.imread("utils/file_oss_utils/img/tip/tip.jpg")  # 加载过程的图片
        ...
        while True:
            for path, img, im0s, vid_cap in self.dataset:
                ...
                for b in range(0, img.shape[0]):
                    mask = np.zeros([img[b].shape[1], img[b].shape[2]], dtype=np.uint8)  # 定义全零掩码mask
                    # mask[round(img[b].shape[1] * hl1):img[b].shape[1], round(img[b].shape[2] * wl1):img[b].shape[2]] = 255
                    pts = np.array([[int(img[b].shape[2] * self.location[1]), int(img[b].shape[1] * self.location[0])],  # pts1
                                    [int(img[b].shape[2] * self.location[3]), int(img[b].shape[1] * self.location[2])],  # pts2
                                    [int(img[b].shape[2] * self.location[5]), int(img[b].shape[1] * self.location[4])],  # pts3
                                    [int(img[b].shape[2] * self.location[7]), int(img[b].shape[1] * self.location[6])]], np.int32)
                ...
                for i, det in enumerate(pred):
                    ...
                    if len(det):
                        ...
                        if t - self.alert_count_time >= 5:
                            ...
                            # 上传到oss
                            # 要上传图片所在的文件夹
                            local_dir_path = 'utils/file_oss_utils/img/area'
                            # 图片对应的path，对应上传到的文件夹
                            elderly_path = "/img/" + "area"
                            img_path = \
                            ...
                            # 向数据库插入数据
                            cv_event = Event(event_type=1,
                                             event_date=event_date,
                                             event_location=self.event_location,
                                             event_desc="检测到有人进入禁区",
                                             event_img=img_path)
                            session.add(cv_event)

                            # 提交
                            session.flush()
                            cv2.imwrite('utils/file_oss_utils/img/area/' + str(time_str) + '.jpg', im0)
                            # 提交
                            session.commit()
                            # 关闭session
                            session.close()
                            ...
                            # cv2.imwrite('utils/file_oss_utils/img/area/' + str(cv_event.event_id) + '.jpg', img_rd)
                            # cv2.imwrite(os.path.join(r"utils/file_oss_utils/img/area/", time_str + '.jpg'), im0)
                            # print("保存截图")
                            self.warn = True
                self.img_det = im0
