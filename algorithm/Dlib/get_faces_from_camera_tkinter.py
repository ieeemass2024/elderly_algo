...
class Face_Register:
    def __init__(self):
        ...
        self.label_face_cnt = tk.Label(self.frame_right_info, text="已录入人数: ")
    ...
    #     self.log_all["text"] = "已清除人脸图像及特征信息"

    def drawChineseText(self, x, y):
        ...
        if self.count < 10:
            ...
            text = "录入完毕，可继续完成录入或按Q退出"
        text_position = (x, y-50)
        text_color = (0, 256, 256)
        draw.text(text_position, text, font=font, fill=text_color)

        # 将PIL图像转换为OpenCV图像
        self.current_frame = np.array(pil_image)

    def chooseElderlyRole(self):
        self.role = "elderly"
        self.log_all["text"] = "选择的角色为 老人"

    def chooseVolunteerRole(self):
        self.role = "volunteer"
        self.log_all["text"] = "选择的角色为 义工"

    def GUI_get_input_name(self):
        if self.role == "":
            self.log_all["text"] = "请先选择角色!"
        else:
    ...
    def GUI_info(self):
        ...
        tk.Label(self.frame_right_info,
                 text="已保存人员数: ").grid(row=2, column=0, columnspan=2, sticky=tk.W, padx=5, pady=2)
        self.label_cnt_face_in_database.grid(row=2, column=2, columnspan=3, sticky=tk.W, padx=5, pady=2)

        tk.Label(self.frame_right_info,
                 text="当前帧人脸数: ").grid(row=3, column=0, columnspan=2, sticky=tk.W, padx=5, pady=2)
        ...
        tk.Label(self.frame_right_info,
                 ...
                 text="Step 1: 选择录入角色").grid(row=5, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)
        tk.Button(self.frame_right_info,
                  text='老人',
                  command=self.chooseElderlyRole).grid(row=6, column=0, columnspan=3, sticky=tk.W, padx=5, pady=2)
        tk.Button(self.frame_right_info,
                  text='义工',
                  command=self.chooseVolunteerRole).grid(row=6, column=2, columnspan=3, sticky=tk.W, padx=5, pady=2)

        # Step 2: Input name and create folders for face
        tk.Label(self.frame_right_info,
                 font=self.font_step_title,
                 text="Step 2: 输入录入人名").grid(row=7, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)
        ...
        tk.Label(self.frame_right_info,
                 font=self.font_step_title,
                 text="Step 3: 保存图像").grid(row=9, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)
    ...
    def create_face_folder(self):
        ...
        if self.input_name_char:
            self.current_face_dir = self.path_photos_from_camera + \
                                    self.role + "_" + str(self.existing_faces_cnt) + "_" + \
                                    self.input_name_char
        else:
            self.current_face_dir = self.path_photos_from_camera + \
                                    self.role + "_" + str(self.existing_faces_cnt)
        os.makedirs(self.current_face_dir)
        self.log_all["text"] = "\"" + self.current_face_dir + "/\" 保存成功!"
        logging.info("\n%-40s %s", "新建的人脸文件夹 / Create folders:", self.current_face_dir)

        self.ss_cnt = 0  # 将人脸计数器清零 / Clear the cnt of screenshots
        ...
        if self.face_folder_created_flag:
            if self.current_frame_faces_cnt == 1:
                if not self.out_of_range_flag:
                    ...
                    self.log_all["text"] = "请保持人脸图像在正中心!"
            else:
                self.log_all["text"] = "当前帧无人脸图像!"
        else:
            self.log_all["text"] = "请先输入录入人姓名!(Step 2)"
            ...
            if len(faces) != 0:
                ...
                for k, d in enumerate(faces):
                    ...
                    self.current_frame = cv2.rectangle(self.current_frame,
                                                       ...
                                                       color_rectangle, 2)
                    self.drawChineseText(d.left(), d.top() - self.hh)
